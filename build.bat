...
if errorlevel 1 (
  echo Error: No se encontro 'java'. Instala JDK 17+ y agrega %JAVA_HOME%\bin al PATH.
  exit /b 1
)
javac -version >NUL 2>&1
if errorlevel 1 (
  echo Error: No se encontro 'javac'. Instala JDK 17+ y agrega %JAVA_HOME%\bin al PATH.
  exit /b 1
)
jar --version >NUL 2>&1
if errorlevel 1 (
  echo Error: No se encontro 'jar'. Asegurate de usar la distribucion JDK (no solo JRE).
  exit /b 1
)
...
mkdir out
if errorlevel 1 (
  echo Error creando carpeta out
  exit /b 1
)
...
(
  ...
  echo Main-Class: labturnos.MainFrame
)> out\MANIFEST.MF
